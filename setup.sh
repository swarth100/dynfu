...
then
    ...
    if ! $QUIET
    then
        cmake -DCMAKE_PREFIX_PATH=/vol/project/2017/362/g1736211/share/OpenCV -DBUILD_TESTS=ON -DCMAKE_CXX_FLAGS="-std=c++11" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON .. || (cd ../ && return 1)
    else
        cmake -DCMAKE_PREFIX_PATH=/vol/project/2017/362/g1736211/share/OpenCV -DBUILD_TESTS=ON -DCMAKE_CXX_FLAGS="-std=c++11" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON .. >/dev/null 2>&1 || (cd ../ && return 1)
    fi
fi
