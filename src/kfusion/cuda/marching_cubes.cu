...
struct CubeIndexEstimator {
    ...
    __device__ __forceinline__ void readTsdf(int x, int y, int z, float& tsdf, int& weight) const {
        unpack_tsdf(*volume, weight);
    }

    __device__ __forceinline__ int computeCubeIndex(int x, int y, int z, float f[8]) const {
        int weight;
...
struct OccupiedVoxels : public CubeIndexEstimator {
    ...
    __device__ __forceinline__ void operator()() const {
        ...
        if (__all(x >= 128) || __all(y >= 128))
            return;
#else
        if (Emulation::All(x >= 128, cta_buffer) || Emulation::All(y >= 128, cta_buffer))
            return;
#endif
        ...
        for (int z = 0; z < 128 - 1; z++) {
            ...
            if (old_global_voxels_count + offs < max_size && numVerts > 0) {
                voxels_indices[old_global_voxels_count + offs]  = 128 * 128 * z + 128 * y + x;
...
int kfusion::device::getOccupiedVoxels(ushort2* const volume, DeviceArray2D<int>& occupied_voxels) {
    OccupiedVoxels ov;
    ov.volume = volume;
    ...
    dim3 grid(divUp(128, block.x), divUp(128, block.y));
...
struct TrianglesGenerator : public CubeIndexEstimator {
    ...
    __device__ __forceinline__ float3 getNodeCoo(int x, int y, int z) const {
    ...
    __device__ __forceinline__ float3 vertex_interp(float3 p0, float3 p1, float f0, float f1) const {
    ...
    __device__ __forceinline__ void operator()() const {
        ...
        int z = voxel / (128 * 128);
        int y = (voxel - z * 128 * 128) / 128;
        int x = (voxel - z * 128 * 128) - y * 128;
        ...
        for (int i = 0; i < numVerts; i += 3) {
            ...
            /* FIXME (dig15): type of output incomaptible with pointtype */
    ...
    __device__ __forceinline__ void store_point(float4* ptr, int index, const float3& point) const {
...
void kfusion::device::generateTriangles(ushort2* const volume, const DeviceArray2D<int>& occupied_voxels,
    ...
    tg.cell_size.x     = volume_size.x / 128; /* FIXME (dig15): divide by the voxel no. instead of hardcoded values */
    tg.cell_size.y     = volume_size.y / 128;
    tg.cell_size.z     = volume_size.z / 128;
    tg.output          = output;
