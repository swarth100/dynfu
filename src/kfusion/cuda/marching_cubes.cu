    ...
    struct OccupiedVoxels : public CubeIndexEstimator
    {
      ...
      operator () () const
      {
        ...
        if (__all (x >= 128) || __all (y >= 128))
          return;
#else
        if (Emulation::All(x >= 128, cta_buffer) ||
            Emulation::All(y >= 128, cta_buffer))
            return;
#endif
...
int
kfusion::device::getOccupiedVoxels (ushort2 *const volume, DeviceArray2D<int>& occupied_voxels)
{
  OccupiedVoxels ov;
  ov.volume = volume;
...
int
kfusion::device::computeOffsetsAndTotalVertices (DeviceArray2D<int>& occupied_voxels)
{
...
void
kfusion::device::generateTriangles (ushort2 *const volume, const DeviceArray2D<int>& occupied_voxels, const float3& volume_size, DeviceArray<PointType>& output)
{
  int device;
